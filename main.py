...
parser = argparse.ArgumentParser(description='NovaCrawl - SEO Spider Tool')
...
LOCAL_MODE = True  # Always local mode in this build — no login required
...
app.secret_key = 'NovaCrawl-secret-key-change-in-production'  # TODO: Use environment variable in production
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        # In local mode, auto-login if not already logged in as a real user
        if LOCAL_MODE and not session.get('user_id'):
...
def get_session_settings():
    ...
    with instances_lock:
        ...
        if session_id not in crawler_instances:
            ...
            # Update last accessed time
            crawler_instances[session_id]['last_accessed'] = datetime.now()
            # Sync user_id onto existing settings manager in case auto-login ran after it was created
            existing_settings = crawler_instances[session_id]['settings']
            if user_id and existing_settings.user_id != user_id:
                existing_settings.user_id = user_id
                existing_settings.tier = tier
                existing_settings.current_settings = existing_settings.load_settings()
...
def generate_xml_export(urls, fields):
    ...
    root = ET.Element('NovaCrawl_export')
...
def generate_xlsx_export(urls, links, issues, fields):
    """Generate a multi-sheet Excel workbook covering all data tabs."""
    import openpyxl
    ...
    def _auto_width(ws, min_w=12, max_w=80):
    ...
    def _val(obj, key):
        """Safely get a value as a clean string."""
        v = obj.get(key, '')
        if isinstance(v, (list, dict)):
            return str(v)
        return '' if v is None else str(v)

    # ── Sheet 1: Overview (all crawled URLs) ──────────────────────────────
    ws_ov = wb.active
    ws_ov.title = 'Overview'
    ov_fields = [f for f in (fields or []) if f not in ('issues_detected', 'links_detailed')] or [
        'url', 'status_code', 'title', 'meta_description', 'h1',
        'word_count', 'response_time', 'content_type', 'canonical_url',
        'lang', 'robots', 'internal_links', 'external_links', 'depth',
    ]
    ws_ov.append([f.replace('_', ' ').title() for f in ov_fields])
    _style_header_row(ws_ov)
    for u in urls:
        ws_ov.append([_val(u, f) for f in ov_fields])
    _auto_width(ws_ov)

    # ── Sheet 2: Internal Links ───────────────────────────────────────────
    ws_int = wb.create_sheet('Internal Links')
    ws_int.append(['Source URL', 'Target URL', 'Anchor Text', 'Target Status Code', 'Placement'])
    _style_header_row(ws_int)
    for lk in links:
        if lk.get('is_internal'):
            ws_int.append([
                _val(lk, 'source_url'),
                _val(lk, 'target_url'),
                _val(lk, 'anchor_text'),
                _val(lk, 'target_status'),
                _val(lk, 'placement'),
            ])
    _auto_width(ws_int)

    # ── Sheet 3: External Links ───────────────────────────────────────────
    ws_ext = wb.create_sheet('External Links')
    ws_ext.append(['Source URL', 'Target URL', 'Anchor Text', 'Target Domain', 'Target Status Code', 'Placement'])
    _style_header_row(ws_ext)
    for lk in links:
        if not lk.get('is_internal'):
            ws_ext.append([
                _val(lk, 'source_url'),
                _val(lk, 'target_url'),
                _val(lk, 'anchor_text'),
                _val(lk, 'target_domain'),
                _val(lk, 'target_status'),
                _val(lk, 'placement'),
            ])
    _auto_width(ws_ext)
    ...
    # ── Sheet 4: Issues ───────────────────────────────────────────────────
    # issues is a flat list of dicts: {url, type, category, issue, details}
    ws_iss = wb.create_sheet('Issues')
    ws_iss.append(['URL', 'Severity', 'Category', 'Issue', 'Details'])
    _style_header_row(ws_iss)
    for iss in issues:
        ws_iss.append([
            _val(iss, 'url'),
            _val(iss, 'type'),
    ...
    ws_perf.append(['URL', 'Status Code', 'Response Time (s)', 'Word Count',
                    'Content Type', 'Size (bytes)', 'Depth', 'Is Internal'])
    _style_header_row(ws_perf)
    for u in urls:
        ws_perf.append([
            _val(u, 'url'),
            _val(u, 'status_code'),
            _val(u, 'response_time'),
            _val(u, 'word_count'),
            _val(u, 'content_type'),
            _val(u, 'size'),
            _val(u, 'depth'),
    ...
    ws_seo.append(['URL', 'Status Code', 'Title', 'Title Length', 'Meta Description',
                   'Meta Desc Length', 'H1', 'H1 Count', 'Canonical URL', 'Lang',
                   'Robots Meta', 'OG Tag Count', 'JSON-LD Count'])
    _style_header_row(ws_seo)
    for u in urls:
        h1_list = _val(u, 'h1_list') or []
        og_tags = _val(u, 'og_tags') or {}
        json_ld = _val(u, 'json_ld') or []
        title = _val(u, 'title') or ''
        meta_desc = _val(u, 'meta_description') or ''
        ws_seo.append([
            _val(u, 'url'),
            _val(u, 'status_code'),
            title,
            ...
            len(meta_desc),
            _val(u, 'h1'),
            len(h1_list) if isinstance(h1_list, list) else 0,
            _val(u, 'canonical_url'),
            _val(u, 'lang'),
            _val(u, 'robots'),
            len(og_tags) if isinstance(og_tags, dict) else 0,
            len(json_ld) if isinstance(json_ld, list) else 0,
        ])
    _auto_width(ws_seo)

    # ── Sheet 7: Content Analysis ─────────────────────────────────────────
    ws_content = wb.create_sheet('Content Analysis')
    ws_content.append(['URL', 'Status Code', 'Word Count', 'Readability Score',
                       'H1', 'H1 Count', 'Content Type', 'Language'])
    _style_header_row(ws_content)
    for u in urls:
        h1_list = _val(u, 'h1_list') or []
        ws_content.append([
            _val(u, 'url'),
            _val(u, 'status_code'),
            _val(u, 'word_count'),
            _val(u, 'readability_score'),
            _val(u, 'h1'),
            len(h1_list) if isinstance(h1_list, list) else 0,
            _val(u, 'content_type'),
            _val(u, 'lang'),
        ])
    _auto_width(ws_content)

    # ── Sheet 8: Technical Analysis ───────────────────────────────────────
    ws_tech = wb.create_sheet('Technical Analysis')
    ws_tech.append(['URL', 'Status Code', 'Response Time (ms)', 'Size (bytes)',
                    'Depth', 'Redirect Depth', 'Redirect Chain', 'Cookie Consent',
                    'Analytics', 'Robots Meta'])
    _style_header_row(ws_tech)
    for u in urls:
        chain = _val(u, 'redirect_chain') or []
        chain_str = ' → '.join(chain) if isinstance(chain, list) else ''
        analytics = _val(u, 'analytics') or {}
        analytics_str = ', '.join(analytics.keys()) if isinstance(analytics, dict) else ''
        ws_tech.append([
            _val(u, 'url'),
            _val(u, 'status_code'),
            _val(u, 'response_time'),
            _val(u, 'size'),
            _val(u, 'depth'),
            _val(u, 'redirect_depth'),
            chain_str,
            _val(u, 'cookie_consent'),
            analytics_str,
            _val(u, 'robots'),
        ])
    _auto_width(ws_tech)
...
def verify_email():
    ...
    if success:
        if app_source == 'workshop':
            redirect_url = os.getenv('WORKSHOP_APP_URL', 'https://workshop.NovaCrawl.com')
        else:
...
def user_info():
    ...
    tier = 'admin' if LOCAL_MODE else session.get('tier', 'guest')
...
def _get_saves_dir():
    """Return the saves directory path, creating it if needed."""
    import sys as _sys
...
@login_required
def list_saves():
    """List all internally saved crawls."""
    try:
        ...
        for fname in sorted(os.listdir(saves_dir), reverse=True):
            ...
            try:
                ...
                saves.append({
                    ...
                    'urls_crawled': meta.get('stats', {}).get('crawled', 0),
...
@login_required
def save_crawl_internal():
    """Save current crawl to internal saves folder."""
    try:
        data = request.get_json()
        saves_dir = _get_saves_dir()

        base_url = data.get('baseUrl', 'crawl')
        try:
            domain = base_url.split('//')[-1].split('/')[0].replace('www.', '').split(':')[0] or 'crawl'
        except Exception:
            domain = 'crawl'
        ...
        if custom_name:
            ...
            import re as _re
            safe = _re.sub(r'[^\w\-.]', '_', custom_name)
            filename = f"{safe}.json" if not safe.endswith('.json') else safe
        else:
            filename = f"{domain}_NovaCrawl_{timestamp}.json"
...
@app.route('/api/saves/<filename>', methods=['GET'])
@login_required
def load_save_internal(filename):
    """Load a saved crawl from internal saves folder."""
    try:
...
@login_required
def delete_save_internal(filename):
    """Delete a saved crawl from internal saves folder."""
    try:
        if '/' in filename or '\\' in filename or '..' in filename:
            return jsonify({'success': False, 'error': 'Invalid filename'}), 400
        saves_dir = _get_saves_dir()
...
def export_data():
    try:
        ...
        # Derive domain for file naming
        try:
            _base = (urls[0].get('url', '') if urls else '') or ''
            _domain = _base.split('//')[-1].split('/')[0].replace('www.', '').split(':')[0] or 'crawl'
        except Exception:
            _domain = 'crawl'
        _date = time.strftime('%Y%m%d')
        def _export_name(suffix, ext):
            return f"{_domain}_NovaCrawl_{_date}_{suffix}.{ext}"
        ...
        # XLSX: one workbook with all sheets — ignore other per-section flags
        if export_format == 'xlsx':
            xlsx_bytes = generate_xlsx_export(urls, links, issues, regular_fields)
            import base64
            return jsonify({
                'success': True,
                'content_b64': base64.b64encode(xlsx_bytes).decode(),
                'mimetype': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                'filename': _export_name('report', 'xlsx'),
                'is_binary': True,
            })
...
def main():
    ...
    print("Starting NovaCrawl on http://localhost:5000")

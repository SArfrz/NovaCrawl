...
class SEOExtractor:
    ...
    def extract_basic_seo_data(soup, result):
        ...
        # Extract headings — capture ALL h1s for multiple-H1 detection
    ...
    def extract_json_ld(soup, result):
        """Extract JSON-LD structured data and validate key schemas."""
        json_ld_scripts = soup.find_all('script', attrs={'type': 'application/ld+json'})

        content_types = {'Article', 'BlogPosting', 'NewsArticle', 'TechArticle', 'Report'}

        for script in json_ld_scripts:
            try:
                raw = json.loads(script.string)
                # Flatten @graph arrays and top-level lists
                ...
                for node in nodes:
                    if not isinstance(node, dict):
                        continue
                    result['json_ld'].append(node)

                    # Pull author and validate content-type schemas
                    schema_type = node.get('@type', '')
                    types = [schema_type] if isinstance(schema_type, str) else (schema_type or [])
                    if any(t in content_types for t in types):
                        # Extract author
                        author = node.get('author')
                        if author and not result.get('meta_author'):
                            if isinstance(author, dict):
                                ...
                                name = str(author)
                            if name:
                                result['meta_author'] = name

                        # Schema validation — check required fields
                        if not result.get('schema_missing_fields'):
                            missing = []
                            if not node.get('headline') and not node.get('name'):
    ...
    @staticmethod
    def extract_author(soup, result):
        """Detect author from HTML byline patterns when meta_author is absent."""
        if result.get('meta_author'):
            return  # already found via meta or JSON-LD

        # Common byline CSS class/attribute patterns
        byline_selectors = [
            {'class': re.compile(r'\bauthor\b|\bbyline\b|\bby-line\b', re.I)},
            {'itemprop': 'author'},
            {'rel': 'author'},
        ]
        for attrs in byline_selectors:
            tag = soup.find(True, attrs=attrs)
            if tag:
                text = tag.get_text(separator=' ').strip()
                # Strip common prefixes like "By John Smith" → "John Smith"
                text = re.sub(r'^(by|written by|author[:\s]*)', '', text, flags=re.I).strip()
                if text and len(text) < 100:
                    result['meta_author'] = text
                    return

        # article:author meta tag (Open Graph variant)
        og_author = soup.find('meta', attrs={'property': 'article:author'})
        if og_author and og_author.get('content'):
            result['meta_author'] = og_author['content'].strip()
    ...
    @staticmethod
    def extract_cookie_consent(soup, html_content, result):
        """Detect cookie consent platform scripts."""
        consent_signatures = {
            'Cookiebot': ['cookiebot', 'CookieConsent'],
            'OneTrust': ['onetrust', 'OptanonConsent'],
            'TrustArc': ['trustarc', 'truste.eu'],
            'CookieYes': ['cookieyes', 'cky-consent'],
            'Usercentrics': ['usercentrics'],
            'Wix Consent': ['wix-consent', 'BoaConsentManager'],
            'GDPR Cookie Consent': ['gdpr-cookie-consent'],
        }
        detected = []
        for name, patterns in consent_signatures.items():
            for pattern in patterns:
                if pattern.lower() in html_content.lower():
                    detected.append(name)
                    break
        result['cookie_consent'] = detected[0] if detected else None

    @staticmethod
    def extract_readability(soup, result):
        """Calculate Flesch reading ease score."""
        try:
            import textstat
            # Use body text only, strip nav/header/footer noise
            for tag in soup(['nav', 'header', 'footer', 'script', 'style']):
                tag.decompose()
            text = soup.get_text(separator=' ')
            text = ' '.join(text.split())
            if len(text) > 100:
                result['readability_score'] = round(textstat.flesch_reading_ease(text), 1)
        except Exception:
            pass

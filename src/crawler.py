...
class WebCrawler:
    ...
    def __init__(self, crawl_id=None, resume_from_db=False):
        ...
        self.session.headers.update({
            'User-Agent': 'SArfrzCrawl/1.0 (Web Crawler)'
        })
    ...
    def _get_default_config(self):
        ...
        return {
            ...
            'user_agent': 'SArfrzCrawl/1.0 (Web Crawler)',
            'timeout': 10,
            'retries': 3,
            ...
            'js_browser': 'chromium',
            'js_headless': True,
            'js_user_agent': 'SArfrzCrawl/1.0 (Web Crawler with JavaScript)',
    ...
    def start_crawl(self, url, user_id=None, session_id=None):
        ...
        try:
            ...
            # Run domain-level checks (llms.txt, AI bot robots.txt) — quick, one-time
            if self.issue_detector:
                try:
                    self.issue_detector.detect_domain_checks(url, self.session)
                except Exception:
                    pass
    ...
    def _crawl_worker(self):
        ...
        if not self._demo_limit_reached:
            ...
            # Run cross-page issue detection (requires full crawl data + link graph)
            if self.issue_detector and self.link_manager:
                print("Running cross-page issue detection...")
                all_links = self.link_manager.all_links.copy()
                self.issue_detector.detect_duplicate_titles_and_meta(self.crawl_results)
                self.issue_detector.detect_broken_link_source_issues(self.crawl_results, all_links)
                self.issue_detector.detect_orphan_pages(self.crawl_results, all_links)
                print(f"Cross-page detection complete. Total issues: {len(self.issue_detector.get_issues())}")
    ...
    def _crawl_url_with_requests(self, url, depth):
        ...
        try:
            ...
            if response.history:
                ...
                result['redirect_depth'] = len(response.history)
            ...
            if 'text/html' in response.headers.get('content-type', ''):
                ...
                self.seo_extractor.extract_cookie_consent(soup, response.text, result)
                self.seo_extractor.extract_readability(soup, result)
                self.seo_extractor.extract_author(soup, result)
    ...
    async def _crawl_async_with_js(self):
        ...
        try:
            ...
            if not self._demo_limit_reached:
                ...
                # Run cross-page issue detection
                if self.issue_detector and self.link_manager:
                    print("Running cross-page issue detection...")
                    all_links = self.link_manager.all_links.copy()
                    self.issue_detector.detect_duplicate_titles_and_meta(self.crawl_results)

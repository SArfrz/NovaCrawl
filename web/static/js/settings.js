...
let defaultSettings = {
    ...
    // Request settings
    userAgent: 'NovaCrawl/1.0 (Web Crawler)',
    timeout: 10,
    retries: 3,
    ...
    // Export settings
    exportFormat: 'xlsx',
    exportFields: ['word_count', 'response_time', 'canonical_url', 'lang'],
    ...
    jsBrowser: 'chromium',
    jsHeadless: true,
    jsUserAgent: 'NovaCrawl/1.0 (Web Crawler with JavaScript)',
...
async function openSettings() {
    // Get user tier info — default to admin (local mode has no real guests)
    let userTier = 'admin';
    try {
    ...
    // Block guests from accessing settings (only applies in multi-user server mode)
    if (userTier === 'guest') {
...
function saveSettings() {
    ...
    // Always enforce robots.txt respect in the GitHub/open-source build
    newSettings.respectRobotsTxt = true;
    ...
    try {
        localStorage.setItem('NovaCrawl_settings', JSON.stringify(newSettings));
...
function resetSettings() {
    if (confirm('Are you sure you want to reset all settings to their default values?')) {
        ...
        // Clear localStorage
        try {
            localStorage.removeItem('NovaCrawl_settings');
...
function validateSettings(settings) {
    ...
    // Core fields (url, status_code, title, meta_description, h1) are always exported
    // so exportFields can be empty — no validation needed here
...
function loadSettings() {
    // Always load from server (SQLite) first — localStorage is unreliable
    // when running under flaskwebgui (new temp profile each launch).
    fetch('/api/get_settings')
        .then(response => response.json())
        .then(data => {
            if (data.success && data.settings && Object.keys(data.settings).length > 0) {
                const merged = { ...defaultSettings, ...data.settings };
                // If saved exportFields is empty, fall back to defaults
                if (!merged.exportFields || merged.exportFields.length === 0) {
                    merged.exportFields = defaultSettings.exportFields;
                }
                currentSettings = merged;
                console.log('Settings loaded from server');
            } else {
                currentSettings = { ...defaultSettings };
                console.warn('No server settings found, using defaults');
            }
            // Cache in localStorage for this session only
            try { localStorage.setItem('NovaCrawl_settings', JSON.stringify(currentSettings)); } catch(e) {}
            applyCustomCSS();
            syncSettingsToBackend();
        })
        .catch(error => {
            console.error('Error loading settings from server:', error);
            // Fallback: localStorage or hardcoded defaults
            try {
                const saved = localStorage.getItem('NovaCrawl_settings');
...
function exportSettings() {
    ...
    a.href = url;
    a.download = 'NovaCrawl-settings.json';

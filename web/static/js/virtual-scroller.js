...
class VirtualScroller {
    constructor(container, options = {}) {
        ...
        // Configuration
        this.rowHeight = options.rowHeight || 40; // px per row — will be auto-measured after first render
    ...
    setupVirtualScroll() {
        ...
        // Cache thead for JS-driven sticky
    ...
    setData(data) {
        ...
        // Always re-read containerHeight when data is set — the container
        // may now be visible even if it wasn't during construction
    ...
    handleScroll() {
        ...
        // Refresh containerHeight on every scroll — fixes cases where
        // ResizeObserver fires before the tab/container is visible (clientHeight=0)
        const h = this.container.clientHeight;
        if (h > 0 && h !== this.containerHeight) {
            this.containerHeight = h;
        }
        // JS-driven sticky header: translate thead down by scrollTop
        // so it always appears at the top of the visible area
        if (this.thead) {
            this.thead.style.transform = `translateY(${this.scrollTop}px)`;
        }
        this.render();
    }
    ...
    render() {
        ...
        // Auto-measure actual row height after first render so spacers are accurate
        if (!this.rowHeightMeasured) {
            const firstRow = this.tableBody.querySelector('tr[data-index]');
            if (firstRow) {
                const measured = firstRow.getBoundingClientRect().height;
                if (measured > 0) {
                    this.rowHeight = measured;
                    this.rowHeightMeasured = true;
                    // Recalculate spacers with real height
                    this.topSpacer.firstChild.style.height = (start * this.rowHeight) + 'px';
                    this.bottomSpacer.firstChild.style.height = ((this.data.length - end) * this.rowHeight) + 'px';
                }
            }
        }
    }
